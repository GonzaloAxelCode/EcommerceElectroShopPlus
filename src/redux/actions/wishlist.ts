...
export const get_wishlist_items = () => async (dispatch: any) => {
...
export const add_wishlist_item =
  (product_id: number) => async (dispatch: any) => {
    if (localStorage.getItem("access")) {
      const config = {
        headers: {
          Authorization: `JWT ${localStorage.getItem("access")}`,
...
export const get_wishlist_item_total = () => async (dispatch: any) => {
  if (localStorage.getItem("access")) {
    const config = {
      headers: {
        Authorization: `JWT ${localStorage.getItem("access")}`,
...
export const remove_wishlist_item =
  (product_id: number) => async (dispatch: any) => {
    if (localStorage.getItem("access")) {
      const body = JSON.stringify({
        product_id,
...
export const clear_wishlist = () => (dispatch: any) => {
  dispatch({
    type: CLEAR_WISHLIST,
  });
};
